...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const calculateInterval = (goal, duration) => {
    return duration / goal;
  };

  const startCounter = (element, duration) => {
    let counter = 0;
    let goal = parseInt(element.dataset.goal);
    let intervalTime = calculateInterval(goal, duration);
  ...
  const observerCallback = (entries, observer) => {
    entries.forEach((entry) => {
      if (entry.isIntersecting) {
        const duration = 2000; // Set the duration for all counters to finish
        numbers.forEach((number) => startCounter(number, duration));
        observer.unobserve(entry.target); // Stop observing the elements after starting the counter
      }
    });
  };
